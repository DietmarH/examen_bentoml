...
def load_test_data():
    ...
    # Adjust path since we're now in tests/integration/
...
def login_user(username="user", password="user123"):
    ...
    response = requests.post(f"{BASE_URL}/login", headers=API_HEADERS, json=login_data)
...
def test_batch_prediction(token, batch_data, expected_results=None):
    ...
    response = requests.post(f"{BASE_URL}/predict_batch", headers=headers, json=payload)
...
def main():
    ...
    for i in range(5):
        ...
        print(
            f"   Input: GRE={student['GRE Score']}, TOEFL={student['TOEFL Score']}, "
