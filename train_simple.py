...
def main():
    ...
    try:
        ...
        X_train = pd.read_csv('/home/ubuntu/examen_bentoml/data/processed/X_train.csv')
        X_test = pd.read_csv('/home/ubuntu/examen_bentoml/data/processed/X_test.csv')
        y_train = pd.read_csv('/home/ubuntu/examen_bentoml/data/processed/y_train.csv').squeeze()
        y_test = pd.read_csv('/home/ubuntu/examen_bentoml/data/processed/y_test.csv').squeeze()
        ...
        for name, model in models.items():
            ...
            log.info(f"{name} CV R²: {mean_cv:.4f} (±{cv_scores.std():.4f})")
        ...
        if test_r2 >= 0.7:
            log.info(f"Model performance satisfactory (R² = {test_r2:.4f})")
            ...
            log.info(f"Verification prediction: {verification_pred[0]:.4f}")
            log.info("Model verification successful!")
            
            return model_tag
        else:
            log.warning(f"Model performance not satisfactory (R² = {test_r2:.4f} < 0.7)")
